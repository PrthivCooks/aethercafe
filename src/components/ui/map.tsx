...
export function WorldMap({ 
  dots = [], 
  lineColor = "#D4AF37", // Updated to Aether Cafe gold color
  showLabels = true,
  ...
  const svgMap = useMemo(
    () => map.getSVG({
      radius: 0.22,
      color: theme === "dark" || true ? "#D4AF3740" : "#00000040", // Force dark mode colors for cafe theme
      shape: "circle",
      backgroundColor: theme === "dark" || true ? "transparent" : "white",
    }),
    [map, theme]
  );
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 bg-[#120F0D] text-[#D4AF37] px-4 py-2 rounded-lg text-sm font-bold tracking-wider uppercase backdrop-blur-sm border border-white/10 shadow-2xl pointer-events-none z-50"
          >
            {hoveredLocation}
          </motion.div>
        )}
